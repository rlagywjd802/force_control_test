...
int main(int argc, char** argv) {
  ...
  // ros
  ros::init(argc, argv, "cartesian_impedance_circle_test");
  ros::NodeHandle nh;
  ...
  const double translational_stiffness{150.0}; // 150
  const double rotational_stiffness{10.0}; // 10
  // const double translational_stiffness_impedance{150.0};
  Eigen::MatrixXd stiffness(6, 6), damping(6, 6);
  stiffness.setZero();
  stiffness.topLeftCorner(3, 3) << translational_stiffness * Eigen::MatrixXd::Identity(3, 3);
  stiffness.bottomRightCorner(3, 3) << rotational_stiffness * Eigen::MatrixXd::Identity(3, 3);
  // stiffness(2,2) = translational_stiffness_impedance;
  damping.setZero();
  damping.topLeftCorner(3, 3) << 2.0 * sqrt(translational_stiffness) *
                                     Eigen::MatrixXd::Identity(3, 3);
  damping.bottomRightCorner(3, 3) << 2.0 * sqrt(rotational_stiffness) *
                                         Eigen::MatrixXd::Identity(3, 3);
  // damping(2,2) = 2.0 * sqrt(translational_stiffness_impedance);

  std::cout << "translational stiffness: " << translational_stiffness << std::endl;
  std::cout << "rotational stiffness: " << rotational_stiffness << std::endl;
