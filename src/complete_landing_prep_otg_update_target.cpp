...
int main(int argc, char** argv) {
  ...
  double max_vel[3] = {0.5, 0.5, 0.5}; 
  double max_acc[3] = {0.5, 0.5, 0.5}; 
  double max_jerk[3] = {0.1, 0.1, 0.1};
    ...
    while (running) {
      ...
      if (print_data.mutex.try_lock()) {
        if (print_data.has_data) {
          ...
          std::cout << print_data.robot_state.O_T_EE[12] << "," << print_data.robot_state.O_T_EE[13] << "," << print_data.robot_state.O_T_EE[14] << ","
                    << print_data.robot_state.O_dP_EE_c[0] << "," << print_data.robot_state.O_dP_EE_c[1] << "," << print_data.robot_state.O_dP_EE_c[2] << ","
                    << print_data.robot_state.O_ddP_EE_c[0] << "," << print_data.robot_state.O_ddP_EE_c[1] << "," << print_data.robot_state.O_ddP_EE_c[2] << std::endl;
  ...
  try {
    ...
    auto cartesian_vel_callback = [=, &running, &print_data, &target, &stage, &t, &prep_t, &takingoff_t0, 
                                    ...
                                    &desired_vel](const franka::RobotState& robot_state, 
                                    franka::Duration period) -> franka::CartesianVelocities 
    {
      ...
      if (stage == OTG) {
        if (!is_otg_started) {
          double* new_ref = target.getTargetValue();
          ...
          prep.applyRefTransform({new_ref[0], new_ref[1], kPrepRefZ});
          takingoff.applyRefTransform({new_ref[0], new_ref[1], kPrepRefZ});
