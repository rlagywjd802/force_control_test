...
class CMotionGenerator {
 public:
  /**
   * Creates a new MotionGenerator instance for a target x.
   *
   * @param[in] speed_factor General speed factor in range [0, 1].
   * @param[in] x_goal Target cartesian positions.
   */
  CMotionGenerator(double speed_factor, const std::array<double, 3> x_goal);
 ...
 private:
  ...
  bool calculateDesiredValues(double t, Vector3d* delta_x_d) const;
  void calculateSynchronizedValues();

  static constexpr double kDeltaXMotionFinished = 1e-6;
  const Vector3d x_goal_;
  ...
  Vector3d dx_max_ = (Vector3d() << 0.2, 0.2, 0.2).finished();
  Vector3d ddx_max_start_ = (Vector3d() << 2.0, 2.0, 2.0).finished();
  Vector3d ddx_max_goal_ = (Vector3d() << 2.0, 2.0, 2.0).finished();
};